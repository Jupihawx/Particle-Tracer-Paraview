...
center_points=(my_data[2][0],my_data[2][1],my_data[2][2])  # to be modified by slider
num_points=int(my_data[2][3]) # to be modified by slider
radius_points= int(my_data[2][4]) # to be modified by slider
...
liveProgrammableSource1.Script = script # replace the value above to the right one for the particle we are working on. i+1 because line 0 is the name of the data in the csv

liveProgrammableSource1.ScriptRequestInformation = ''
liveProgrammableSource1.PythonPath = ''
liveProgrammableSource1.ScriptCheckNeedsUpdate = script_update
...
particleTracerGenerated = ParticleTracer(registrationName='ParticleTracerGenerated1', Input=FieldSource,
...
ColorBy(particleTracerGeneratedDisplay, None)
...
index=int("2")
particleTracerGeneratedDisplay.AmbientColor=[my_data[index][9],my_data[index][10],my_data[index][11]]
particleTracerGeneratedDisplay.DiffuseColor=[my_data[index][9],my_data[index][10],my_data[index][11]]
